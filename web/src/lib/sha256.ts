...
// 非安全上下文（如局域网 IP 明文 HTTP）没有 crypto.subtle，生成链路的
// 去重/幂等哈希退化为纯 JS 实现；仅在 native digest 不可用时走这条路径。
...
export function sha256HexFallback(bytes: Uint8Array): string {
    return Array.from(sha256BytesFallback(bytes), (byte) => byte.toString(16).padStart(2, "0")).join("");
}
