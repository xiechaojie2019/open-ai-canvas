...
| 5 | 资源、素材与存储 | 31 + 3 | 🟡 画布 CRUD + 素材库全量 + 资源分片上传/CRUD + 文件下发（ETag/Range/匿名签名）+ 资源 CRUD（整传/URL 导入/用量/OSS 直链）+ 孤儿资源清理作业已通；OSS 设置/Eagle 待做 |
| 6 | 项目与短剧工作流 | 47 | 🟢 项目 CRUD + 单元/画布链接 + 素材文件夹树 + 角色配音 + 镜头候选 + 素材关联 + 工作台读视图已通 |
